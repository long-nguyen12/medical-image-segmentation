...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--train_path",
        type=str,
        default="./data/dataset/backup/",
        help="path to train dataset",
    )
    parser.add_argument("--train_save", type=str, default="polyp-seg-exp")
    args = parser.parse_args()

    epochs = args.num_epochs
    ds = ["CVC-ClinicDB", "Kvasir-SEG"]
    for ids, _ds in enumerate(ds):
        ...
        for epoch in range(start_epoch, epochs + 1):
            ...
            if epoch % 5 == 0:
                mean_iou, mean_dice, mean_precision, mean_recall = inference(
                    model, f"{args.train_path}/{ds[(ids+1) % 2]}/"
                )
